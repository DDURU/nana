...
public class RepetitiveStatement {
    ...
    /*           
     * <<for문>>
     * - for(int=1 ; i <=10; i++){} 초기화(앞), 조건식(중간), 증감식(마지막)
     * - for(1부터 10까지 1씩 증가한면서 반복){}
		...
		*/
		...
		for(int i = 1; i<= 10; i++){
			...
			System.out.println(i+"번째 반복");//초기화한 변수를 블럭안에서 사용할수 있다.
		}
	...
	sum=0;
	for(int i = 1; i <= 100; i+=2){if (i%2 == 1){
		sum += i;
	} }System.out.println(sum);
	 ...
	 outer : for(int i = 2; i <= 9; i++){
		 for(int j = 1; j<=9; j++){
			 if(j == 5){
				 ...
				 //break outer; // outer라는 이름의 반목문을 빠져나간다.
